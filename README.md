...
**Android Mode** — For any Android device with USB debugging enabled, using ADB + gnirehtet VPN relay:
...
1. Switch to the **Files** tab (next to the Log tab)
2. Click **Add Files** to select files, or drag and drop files onto the window
3. Selected files appear in the list — use **Remove** or **Clear All** to adjust
4. Click **UPLOAD** (enabled when a device is connected and files are selected)
5. If a file already exists on the device, you'll be prompted to overwrite or skip
6. Progress bar shows upload status for each file
...
- **File upload** — push files to connected Android devices via ADB
  - Multi-file selection with add/remove and drag-and-drop support
...
│   ├── gui.py            # Tkinter GUI (tabs, file upload, optional admin features)
│   ├── build_config.py   # Build-time config (enabled modes)
│   ├── file_uploader.py  # File push to /sdcard/Download/
...
├── tests/                # Test scripts
│   ├── test_file_uploader.py    # Unit tests for file push to Download
...
- Python 3.8+
- Dependencies are auto-installed by `build.py` from `requirements.txt` at the start of every build (PyInstaller, tkinterdnd2 for drag-and-drop). Pass `--no-deps` to skip auto-install if you manage your own environment.
...
For `--mode winmobile`, no resource files are required.
...
1. Ensure the device is connected and USB debugging is authorized
2. Check the log panel for specific error messages
3. Large files may take longer — each file has a 5-minute timeout
4. If "device not found" appears, reconnect the device and try again
5. Ensure the device has sufficient storage
