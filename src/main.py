...
def extract_resources() -> dict:
    ...
    Only extracts resources required by the enabled modes (from build_config).
    """
    app_dir = get_app_data_dir()
    android = 'android' in ENABLED_MODES

    # Always extract logo and icon
    resources = {
    ...
    if android:
        resources['gnirehtet'] = app_dir / f'gnirehtet{BIN_EXT}'
        resources['adb'] = app_dir / f'adb{BIN_EXT}'
